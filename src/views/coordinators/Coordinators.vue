<template>
  <div>
    ...
    <CTable hover responsive>
      ...
      <CTableBody>
        <CTableRow v-for="c in filteredCoordinators" :key="c.coordinator_id">
          ...
          <CTableDataCell>
            <CButton size="sm" color="info" @click="openEditModal(c)"
              >Edit</CButton
            >
            <CButton size="sm" color="danger" class="ms-2" @click="onDelete(c)"
              >Delete</CButton
            >
          </CTableDataCell>
        </CTableRow>
    ...
    >
      ...
      <CModalBody
        >Are you sure you want to delete coordinator #<b>{{
          confirmDelete.coordinator?.coordinator_id
        }}</b
        >?</CModalBody
      >
      <CModalFooter>
        <CButton color="secondary" @click="confirmDelete.visible = false"
          >Cancel</CButton
        >
        <CButton color="danger" @click="confirmDeleteCoordinator"
          >Delete</CButton
        >
      </CModalFooter>
    </CModal>
...
const onFormSubmit = async (c) => {
  c.coordinator_id
    ? await updateCoordinator(c.coordinator_id, c)
    : await createCoordinator(c);
  await fetchCoordinators();
  closeModal();
};
const onDelete = (c) => {
  confirmDelete.value = { visible: true, coordinator: c };
};
