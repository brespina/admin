<template>
  <div>
    ...
    <CTable hover responsive>
      ...
      <CTableBody>
        <CTableRow v-for="o in filteredOfficers" :key="o.officer_id">
          ...
          <CTableDataCell>
            <CButton size="sm" color="info" @click="openEditModal(o)">Edit</CButton>
            <CButton size="sm" color="danger" class="ms-2" @click="onDelete(o)">Delete</CButton>
          </CTableDataCell>
        </CTableRow>
      </CTableBody>
    </CTable>
    <OfficerForm :visible="modalVisible" :officer="editingOfficer" @submit="onFormSubmit" @close="closeModal" />
    <CModal :visible="confirmDelete.visible" @close="confirmDelete.visible = false">
      ...
      <CModalBody>Are you sure you want to delete officer #<b>{{ confirmDelete.officer?.officer_id }}</b>?</CModalBody>
      <CModalFooter>
...
import { getOfficers, createOfficer, updateOfficer, deleteOfficer } from '@/services/officers.js'
...
const openAddModal = () => { editingOfficer.value = null; modalVisible.value = true }
const openEditModal = (o) => { editingOfficer.value = { ...o }; modalVisible.value = true }
const closeModal = () => { modalVisible.value = false; editingOfficer.value = null }
const onFormSubmit = async (o) => { o.officer_id ? await updateOfficer(o.officer_id, o) : await createOfficer(o); await fetchOfficers(); closeModal() }
const onDelete = (o) => { confirmDelete.value = { visible: true, officer: o } }
